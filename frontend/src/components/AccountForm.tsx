...
export function AccountForm({ onSubmit, loading, qrResult }: AccountFormProps) {
  ...
  return (
    <form className="space-y-2" onSubmit={e => { e.preventDefault(); onSubmit({ accountNumber, ifsc, name, styling }); }}>
      <Input label="Account Number" value={accountNumber} onChange={e => setAccountNumber(e.target.value)} required />
      <Input label="IFSC" value={ifsc} onChange={e => setIfsc(e.target.value)} required />
